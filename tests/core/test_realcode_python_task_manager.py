...
def test_eval_single_missing_keys(monkeypatch, minimal_local_repo_task, tmp_path):
    ...
    manager = TaskManagerRealcode(
        mode="local", n_jobs=1, gen_columns=["gt", "pass", "return_empty_str", "gen"]
    )
    ...
    class DummyRepo:
        ...
        def run_test(self, command, timeout=300):
            ...
            return {
                "report": {
                    "tests": [{"nodeid": "t1", "outcome": "passed"}],
                    "summary": {"passed": 1},
                }
            }
    ...
    assert (
        "pass_pass" not in task
    ), "pass_pass should not be in task if pass not in original task"
...
def test_eval_single_missing_columns(monkeypatch, tmp_path):
    ...
    assert (
        "pass_gt" not in task
    ), "pass_gt не должен быть в task, если gt не было в исходном task"
    assert (
        "pass_return_empty_str" not in task
    ), "pass_return_empty_str не должен быть в task, если return_empty_str не было в исходном task"
