...
class JavaDockerRepo(AbstractDockerRepo):
    ...
    def local_mvn_folder(self) -> str:
        ...
        """
        return os.path.join(self.cache_folder, ".m2/repository")
    ...
    def build_env(self, command: str, timeout: int = DEFAULT_BUILD_TIMEOUT_INT) -> None:
        """
    ...
    def change_file_realcode(
        self, fn_relative, left_context: str, gt: str, right_context: str
    ):
        ...
        self.change_file(fn_relative, left_context + gt + right_context)

    def run_test(self, command: str, timeout: int = DEFAULT_EVAL_TIMEOUT_INT) -> dict:
        """
        ...
        return {
            ...
            "parser": analyze_maven_stdout(
                stdout=self.stdout, full_path=self.default_cache_folder
            ),
