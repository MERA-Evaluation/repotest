...
class NodejsDockerRepo(AbstractDockerRepo):
    ...
    def _local_npm_cache(self) -> str:
        return os.path.join(self.cache_folder or DEFAULT_CACHE_FOLDER, ".npm_cache")
    ...
    def _parse_npm_test_output(self, output: str) -> Dict[str, int]:
        """Parse npm/jest test output."""
        results = {"passed": 0, "failed": 0, "skipped": 0, "total": 0}
        
        cache_folder = self.cache_folder or DEFAULT_CACHE_FOLDER
        txt_path = os.path.join(cache_folder, "test_results.txt")
