"""
Ruby Docker repository test runner.
"""
...
def parse_ruby_test_output(stdout: str) -> Dict[str, object]:
...
class RubyDockerRepo(AbstractDockerRepo):
    ...
    def __init__(self, repo: str, base_commit: str, default_cache_folder: str = DEFAULT_CACHE_FOLDER,
                 default_url: str = "http://github.com", image_name: str = "ruby:latest",
    ...
    @cached_property
    def _user_ruby_cache(self) -> str:
        return os.path.expanduser("~/.cache/ruby")
    
    @cached_property
    def _local_ruby_cache(self) -> str:
        return os.path.join(self.cache_folder, ".ruby_cache")
    ...
    def _setup_container_volumes(self, workdir: Optional[str] = None) -> Dict[str, Dict[str, str]]:
        ...
        if self.cache_mode == "volume":
            self.create_volume("ruby-cache")
            volumes["ruby-cache"] = {"bind": "/root/.cache/ruby", "mode": "rw"}
        return volumes
    ...
    def run_test(self, command: str = "bundle exec rspec", timeout: int = DEFAULT_EVAL_TIMEOUT_INT,
        ...
        try:
            ...
            self._convert_std_from_bytes_to_str()
        test_results = {}
        fn_result = os.path.join(self.cache_folder, "rspec_results.json")
    ...
    def _format_results(self, test_json: Optional[Dict] = None) -> Dict[str, object]:
        return {"stdout": self.stdout, "stderr": self.stderr, "std": self.std, "returncode": self.return_code,
                "parser": parse_ruby_test_output(self.stdout), "report": test_json or {},
