"""Kotlin language Docker repository management."""
...
DOCKER_KOTLIN_DEFAULT_IMAGE = "gradle:jdk11"
...
class KotlinDockerRepo(AbstractDockerRepo):
    ...
    def __init__(
        ...
        image_name: str = DOCKER_KOTLIN_DEFAULT_IMAGE,
    ...
    def _local_gradle_cache(self) -> str:
        return os.path.join(self.cache_folder or DEFAULT_CACHE_FOLDER, ".gradle_cache")
    ...
    def _parse_gradle_test_output(self, output: str) -> Dict[str, object]:
        """Parse Gradle test output."""
        results = {"passed": 0, "failed": 0, "skipped": 0, "total": 0}
        
        txt_path = os.path.join(self.cache_folder or DEFAULT_CACHE_FOLDER, "test_results.txt")
        if os.path.exists(txt_path):
            try:
                ...
                logger.warning(f"Failed to parse Gradle test results: {e}")
