...
def parse_rust_test_output(stdout: str) -> Dict[str, object]:
...
class RustDockerRepo(AbstractDockerRepo):
    ...
    @cached_property
    def _user_rust_cache(self) -> str:
        return os.path.expanduser("~/.cache/rust")
    ...
    def _setup_container_volumes(self, workdir: Optional[str] = None) -> Dict[str, Dict[str, str]]:
        ...
        if self.cache_mode == "volume":
            self.create_volume("rust-cache")
            volumes["rust-cache"] = {"bind": "/root/.cache/rust", "mode": "rw"}
        return volumes
    ...
    def _format_results(self, test_json: Optional[Dict] = None) -> Dict[str, object]:
        return {"stdout": self.stdout, "stderr": self.stderr, "std": self.std, "returncode": self.return_code,
                "parser": parse_rust_test_output(self.stdout), "report": test_json or {},
