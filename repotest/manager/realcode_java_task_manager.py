...
from repotest.parsers.java.surefire_report import (find_all_test_report_dirs,
                                                   find_test_reports)
...
class JavaEvaluatorRealcode:
    ...
    def get_passed_dict(self, task: dict) -> dict:
        ...
        for key in self.gen_columns:
            if (key in task) or (key == "gen"):
                ...
                res[res_column_name] = int(
                    (passed_current != set()) & ((passed_current & passed) == passed)
                )
        return res
    ...
    def _new_repo(self, task: dict) -> Union[JavaDockerRepo, JavaLocalRepo]:
        try:
            kwargs = (
                {"image_name": task["image_name"]} if self.mode == "docker" else dict()
            )
            repo = self.RepoClass(
                repo=task["repo"], base_commit=task["base_commit"], **kwargs
            )
    ...
    def eval_single(self, task: dict):
        ...
        if self.build_success_status.get(repo_id, 0) == 0:
            logger.info(
                f"[Test failure] Skip {repo_id} because it was unable to build earlier"
            )
            task["status"] = 0
            return None
