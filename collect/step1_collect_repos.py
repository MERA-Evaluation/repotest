...
def collect_repos_from_s3(
    ...
    with open(output_file, output_mode) as out_f, \
        ...
        for date_folder in tqdm(date_folders, desc="Date folders"):
                ...
                while retries < max_retries and not success:
                    try:
                        ...
                        for repo in repos:
                            ...
                            # Detect framework for statistics
                            framework = language_filter.get_detected_framework(repo) if hasattr(language_filter, 'get_detected_framework') else None
                            if framework == 'jest':
                                stats['jest_projects'] += 1
                            elif framework == 'mocha':
                                stats['mocha_projects'] += 1
                            ...
                            # Add detected framework info if available
                            if framework:
                                detailed_repo['_test_framework'] = framework
            ...
            logger.info(
                ...
                f"Jest: {stats['jest_projects']}, Mocha: {stats['mocha_projects']}, "
    ...
    logger.info(f"  Jest projects: {stats['jest_projects']}")
    logger.info(f"  Mocha projects: {stats['mocha_projects']}")
...
def collect_javascript_repos_from_s3(
    ...
    s3_bucket: str = "test",
    s3_prefix: str = "test/test",
    test_system: Literal['jest', 'mocha', 'any'] = 'any',
    aws_profile: Optional[str] = "maintainer",
    github_token: Optional[str] = None,
    start_date: Optional[str] = None,
    end_date: Optional[str] = None,
    ...
    """Convenience function for JavaScript repos with test system selection."""
    from collect.lang.s3_javascript import JavaScriptFilter
    language_filter = JavaScriptFilter(test_system=test_system)
...
def collect_jest_repos_from_s3(
    output_file: str,
    s3_bucket: str = "test",
    s3_prefix: str = "test/test",
    aws_profile: Optional[str] = "maintainer",
    ...
    """Collect ONLY Jest JavaScript repos."""
    collect_javascript_repos_from_s3(
        output_file=output_file,
        s3_bucket=s3_bucket,
        s3_prefix=s3_prefix,
        test_system='jest',
...
def collect_mocha_repos_from_s3(
    output_file: str,
    s3_bucket: str = "test",
    s3_prefix: str = "test/test",
    aws_profile: Optional[str] = "maintainer",
    ...
    """Collect ONLY Mocha JavaScript repos."""
    collect_javascript_repos_from_s3(
        output_file=output_file,
        s3_bucket=s3_bucket,
        s3_prefix=s3_prefix,
...
if __name__ == "__main__":
    fire.Fire({
        ...
        'jest': collect_jest_repos_from_s3,
        'mocha': collect_mocha_repos_from_s3,
