# repotest/pipeline/steps/step6_e2e_execution_jest.py

"""
Step 6: End-to-End Execution — Patch/Test-Patch Application in Docker JS Repos (Jest, No Classes!)

1. For each task:
   - Extract all test files from `test_patch` (ignores binary)
   - Form `command_test_small` using Jest
...
build_command_js_jest = "npm ci --legacy-peer-deps --loglevel=error || npm install --legacy-peer-deps --loglevel=error;npm install jest --save-dev --legacy-peer-deps --loglevel=error"
...
def extract_test(test_result: dict) -> tuple[set, set]:
    """
    Extracts passed and failed test names from a Jest test result dictionary.

    Parameters
    ----------
    test_result : dict
        Dictionary containing Jest JSON report information.
    ...
    failed : set
        Set of test names that failed (including pending).
    """
    passed = set()
    failed = set()
    was = set()

    test_results = test_result.get("testResults", [])
...
def build_and_run_tests_docker(task: dict, delete_log = True) -> dict:
    ...
    Handles all logic for a single task:
      - Extracts test files
      - Forms command_test_small using Jest
    ...
    task["command_test_small"] = f'bash -lc "npx jest {test_files_arg} --json --outputFile=jest-results.json --passWithNoTests --runInBand --forceExit 2>&1 || true"'
    ...
    try:
        ...
        if not repo.was_build:
            dct_build = repo.build_env(
                task.get("command_build", build_command_js_jest),
...
def step6_e2e_execution_jest(
    ...
    Adapted for JavaScript repos using Jest.
    """
